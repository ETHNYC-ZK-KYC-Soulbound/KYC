...
function VerifyButton() {
  ...
  async function handleVerifyClick(event: React.MouseEvent<HTMLButtonElement>) {
    ...
    const apiURL = `${process.env.REACT_APP_LOCAL_API_BASE_URL as string}/create-verification-session`
    const apiBody = {
      // TODO: change this to Worldcoin's output address (unique)
      address: '0xab3f459de154B9D58DE8AdA4b4AEe271823c3155',
    }
    const response = await axios.post<{ clientSecret: string, address: string }>(apiURL, apiBody)
